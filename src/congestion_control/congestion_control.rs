...
pub use mixcc::MixCC;
// pub use mixcc::MixCCConfig;
pub use sqp::SQP;
pub use sqp::SQPConfig;
...
pub enum CongestionControlAlgorithm {
    ...
    /// MixCC is a hybrid congestion control algorithm that combines BBR, CUBIC,
    /// and COPA to leverage the strengths of each algorithm.(Test)
    MixCC,
...
impl FromStr for CongestionControlAlgorithm {
    ...
    fn from_str(algor: &str) -> Result<CongestionControlAlgorithm> {
        ...
        } else if algor.eq_ignore_ascii_case("mixcc") {
            Ok(CongestionControlAlgorithm::MixCC)
        } else if algor.eq_ignore_ascii_case("sqp") {
            Ok(CongestionControlAlgorithm::SQP)
        } else {
...
pub fn build_congestion_controller(conf: &RecoveryConfig) -> Box<dyn CongestionController> {
    match conf.congestion_control_algorithm {
        ...
        CongestionControlAlgorithm::MixCC => {
            println!("Using MixCC Congestion Control Algorithm");
            Box::new(MixCC::new())
        }
        CongestionControlAlgorithm::SQP => {
            println!("Using SQP Congestion Control Algorithm Test");
            Box::new(SQP::new(SQPConfig::from(conf)))
        }
    }
}
