...
impl Connection {
    ...
    pub fn stream_write_sqp(&mut self, stream_id: u64, buf: Bytes, fin: bool) -> Result<usize> {
        let buf_size = buf.len();
        match self.stream_write(stream_id, buf, fin) {
            Ok(written) => {
                let path = self.paths.get_active_mut()?;
                path.recovery.congestion.set_sqp_frame_size(buf_size);
                Ok(written)
            }
            Err(e) => {
                Err(e)
    ...
    pub fn get_sqp_bandwidth_estimate(&mut self) -> Result<u64> {
        let path = self.paths.get_active_mut()?;
        Ok(path.recovery.congestion.get_sqp_bandwidth_estimate().unwrap())
    }
